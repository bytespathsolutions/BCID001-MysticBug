  ...
  const fetchUserRoleData = async (user) => {
    ...
    try {
      if (user.userType === 'doctor') {
        ...
        const response = await fetch(`${BASE_URL}/investments?investorId=${user._id}`)
        const data = await response.json()
        setUserData(data || [])
      } else {
        setUserData([])
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-b bg-[#f7fafc]">
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedUser?.userType === 'doctor' && selectedUser.specialization && (
                  <div>
                        ...
                        <tbody>
                          {userData.map((appointment, index) => (
                            <tr key={appointment._id || index} className="border-t hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3">
                                <span className={`px-2 py-1 rounded text-sm ${appointment.status === 'completed' ? 'bg-green-100 text-green-700' :
                        ...
                        <tbody>
                          {userData.map((investment, index) => (
                            <tr key={investment._id || index} className="border-t hover:bg-gray-50">
                              <td className="px-4 py-3">{index + 1}</td>
                              <td className="px-4 py-3">{investment.projectName || 'N/A'}</td>
                              <td className="px-4 py-3">₹{investment.amount?.toLocaleString() || 'N/A'}</td>
                              <td className="px-4 py-3">
                                {investment.date ? new Date(investment.date).toLocaleDateString() : 'N/A'}
                              </td>
                              <td className="px-4 py-3">
                                <span className={`px-2 py-1 rounded text-sm ${investment.status === 'active' ? 'bg-green-100 text-green-700' :
                                  'bg-gray-100 text-gray-700'
                                  }`}>
